...
from layers.unit import Unit2D
import math
import numpy as np
import time
...
if __name__ == "__main__":
    ...
    from st_gcn import SpatioTemporalModel
    ...
    temporal_transformer = tcn_unit_attention(in_channels, out_channels, dv_factor, dk_factor, Nh, n,
                 relative, only_temporal_attention, dropout, kernel_size_temporal, stride, weight_matrix,
                 last, layer, device, more_channels, drop_connect, num_point)
    ...
    spatio_transformer = SpatioTemporalModel(adj, num_joints_in=17, in_features=32, num_joints_out=32, 
                            filter_widths=[3, 1, 1], channels=128)
    # model = model.cuda()

    print("spatio_transformer:", spatio_transformer)

    model_params = 0

    for parameter in spatio_transformer.parameters():
        model_params += parameter.numel()

    print('INFO: Trainable parameter count:', model_params)

    x = torch.randn((16, 1024, 17, 2))              # (N, T, V, C)
    print("x:", x.shape)
    x = x.permute((0, 3, 1, 2))                 # (N, C, T, V)

    y = temporal_transformer(x)   

    print("y:", y.shape)

    # input = torch.randn(2, 27, 17, 32)             # (16, 32, 5, 17)   (N, C, T, V)
    # input = input.cuda()

    y = y.permute((0, 2, 3, 1))
    # summary(model, (27, 15, 2))
    output = spatio_transformer(y)
    print(output.shape)          # ((2, 1, 17, 3)
