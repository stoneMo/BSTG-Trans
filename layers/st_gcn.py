...
from layers.local_attention import LocalGraph
from layers.global_attention import MultiGlobalGraph, SingleGlobalGraph
...
class GraphAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("x graph attention:", x.shape)
        # x: (B, T, N, C) --> (B, C, T, N)
        x = x.permute(0, 3, 1, 2)
        x = self.relu(self.cat_bn(self.cat_conv(x)))
        # print("x after graph attention:", x.shape)
...
if __name__ == "__main__":
    ...
    model = SpatioTemporalModel(adj, num_joints_in=17, in_features=32, num_joints_out=32, 
